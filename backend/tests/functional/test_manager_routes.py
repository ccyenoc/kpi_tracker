...
@patch("routes.kpi_routes.get_dashboard_stats_service")
...
@patch("routes.kpi_routes.get_all_submissions")
...
@patch("routes.kpi_routes.verify_submission_service")
...
# FT-29: Reject Staff Submission - POST /api/kpi/verify-submission
@patch("routes.kpi_routes.verify_submission_service")
def test_verify_submission_reject_success(mock_verify, client, jwt_mock):
    jwt_mock.return_value = {"user_id": "manager_123"}
    mock_verify.return_value = {
        "success": True,
...
# FT-16: Retrieve KPI History Data - GET /api/manager/kpi/history
@patch("routes.kpi_routes.get_kpi_history")
def test_retrieve_kpi_history_success(mock_history, client):
    mock_history.return_value = {
        "success": True,
        "chart": [
            {"time": "Week 1", "kpi": 50, "progress": 20, "prediction": 25},
            {"time": "Week 2", "kpi": 50, "progress": 30, "prediction": 35},
            {"time": "Week 3", "kpi": 50, "progress": 40, "prediction": 45},
            {"time": "Week 4", "kpi": 50, "progress": 50, "prediction": 55}
        ]
    }
    
    headers = {"Authorization": "Bearer valid_manager_token"}
    response = client.get("/api/manager/kpi/history", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
...
# FT-15: Retrieve KPI Prediction Data - GET /api/manager/kpi/{kpi_id}/predict
@patch("routes.kpi_routes.predict_kpi")
def test_retrieve_kpi_prediction_success(mock_predict, client):
    mock_predict.return_value = {
        "success": True,
        "kpiId": "kpi_001",
        "overallPredictedProgress": 75.0,
        "staffPredictions": [
            {"userId": "staff_01", "name": "Jane Staff", "prediction": 75.0}
        ]
    }
    
    headers = {"Authorization": "Bearer valid_manager_token"}
    response = client.get("/api/manager/kpi/kpi_001/predict", headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert data["success"] is True
    assert data["kpiId"] == "kpi_001"
    assert data["overallPredictedProgress"] == 75.0
